...
class FirewallManager:
    ...
    def __init__(self, interface="eth0"):
        ...
        if os.path.exists(MODEL_FILE):
            ...
            df = self.preparator.generate_synthetic_dataset(n_samples=10000)
    

            df_normalized = self.preparator.normalize_features(df)

            X, y = self.preparator.prepare_data(df_normalized)
            ...
            self.preparator.save_normalization_params()
    ...
    def validate_interface(self, interface):
        ...
        try:
            output = subprocess.run(["netsh", "interface", "show", "interface", f"name={interface}"], capture_output=True, text=True)
